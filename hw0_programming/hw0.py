...
from MyLeastSquare import MyLeastSquare
...
w_ls, error_rate = MyLeastSquare(X,r)
print(w_ls)

# Draw the decision boundary based on the new weigh obtained by solving the least square problem
l=np.max(abs(w_ls)) # scale the vector to match draw a better picture
plt.plot([-w_ls[1],w_ls[1]]/l,[w_ls[0],-w_ls[0]]/l,'k-')

# Plotting
plt.axis('equal')
plt.grid(True)
fig3.set_size_inches(10, 10)
fig3.savefig('least_square.png') # saving the final figure
plt.show()

# Print out the error rate and number of iterations for convergence
print('Error Rate for Least Square: %.2f' %error_rate)       # example of a formatted print
